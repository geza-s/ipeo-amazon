...
def validate(model, dataloader, device, loss_fn=nn.BCEWithLogitsLoss()):
    ...
    with torch.no_grad():
        for i_batch, sample_batch in tqdm(enumerate(dataloader)):
            ...
            acc.append(np.array((predicted == ground_truth)).astype(np.float64).mean())

    return np.stack(tot_loss).mean(), np.stack(acc).mean()
...
def train_epoch(model, dataloader, device, lr=0.01, optimizer=None, loss_fn=nn.BCEWithLogitsLoss()):
    ...
    for i_batch, sample_batch in tqdm(enumerate(dataloader)):
        ...
        if i_batch == 0:
            ...
            print(f"Predicted : {predicted} and calculated accuracy: {np.stack(acc).mean()}")

        if i_batch % 100 == 0:  # print every 100 mini-batches the mean loss up to now
            print(f"loss: {np.stack(total_loss).mean()} and acc: {np.stack(acc).mean()}")

    return np.stack(total_loss).mean(), np.stack(acc).mean()
...
def train(model, train_loader, validation_dataloader, device, optimizer=None, lr=0.01, epochs=2,
    ...
    for ep in range(epochs):
        tl, ta = train_epoch(model, train_loader, optimizer=optimizer, lr=lr, loss_fn=loss_fn, device=device)
